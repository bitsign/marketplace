            ...
            <div class="card shadow">
                ...
                <div class="card-body">
                    <div class="row no-gutters align-items-center">
                        ...
                        <form method="post" action="{{ route('posts.update', $post) }}" class="form-horizontal" role="form"
                            id="post_form">
                            @method('PUT')
                            @csrf
                            <div class="mb-3 row">
                            ...
                            <div class="mb-3 row">
                                <label class="col-lg-2 text-lg-end">{{ __('admin.intro') }} <code>*</code></label>
                                <div class="col-lg-10">
                                    @foreach (config('app.available_locales') as $lang)
                                        <div class="input-group mb-1">
                                            <span class="input-group-text"><img src="{{ url('assets/img/' . $lang . '.png') }}"
                                                    width="18px"></span>
                                            <input name="intro[{{ $lang }}]" type="text"
                                                value="{{ $translations[$lang]['intro'] ?? '' }}" class="form-control"
                                                required>
                                        </div>
                                    @endforeach
                                </div>
                            ...
                            @foreach (config('app.available_locales') as $lang)
                                <div class="mb-3 row">
                                    ...
                                    <div class="col-lg-10">
                                        <textarea id="editor1" class="editor form-control" name="content[{{ $lang }}]" rows="10" cols="80">{{ $translations[$lang]['content'] ?? '' }}</textarea>
                                    </div>
                                </div>
                            @endforeach
                                ...
                                <div class="col-lg-10">
                                    <div class="input-group col-lg-12">
                                        <input id="fieldID" type="text"
                                            value="{{ @!empty($post['image']) ? $post['image'] : '' }}" name="image"
                                            class="form-control">
                                        <span class="input-group-btn">
                                            <a class="btn btn-primary iframe-btn" type="button"
                                                href="{{ URL::to('/assets/admin/plugins/filemanager/dialog.php') }}?type=2&fldr=blog&field_id=fieldID">{{ __('admin.select_image') }}</a>
                                        </span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="mb-3 row">
                                <label class="col-lg-2 text-lg-end">{{ __('tags') }}</label>
                                <div class="col-lg-10">
                                    <select class="form-select select2" name="tags[]" id="tags" multiple>
                                        <option value="">{{ __('tags') }}</option>
                                        @foreach ($terms as $term)
                                            <option value="{{ $term->term_key }}" @selected(in_array($term->term_key, explode(',',$post['tags'])) === true)>{{ $term->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="mb-3 row">
                                <label class="col-lg-2 text-lg-end">{{ __('admin.published') }}?</label>
                                <div class="col-lg-10">
                                    <input type="checkbox" data-onstyle="success" data-offstyle="danger" data-size="mini"
                                        data-on="{{ __('yes') }}" data-off="{{ __('yes') }}"
                                        data-toggle="toggle" name="published" value="1"
                                        {{ !empty($post['published']) || empty($post['id']) ? 'checked' : '' }} />
                                </div>
                            </div>
